...
function doPost(e) {
  try {
    ...
    if (action === "atualizarPedido") return jsonOut(atualizarPedido(body));
  ...
  try {
    ...
    if (lastP >= 2) {
      ...
      for (var i = 0; i < idsP.length; i++) {
        if (String(idsP[i][0]) === String(pedidoId)) { rowIndex = i + 2; break; }
      }
    }
    ...
    shP.getRange(rowIndex, 2).setValue(body.nome || "");
    shP.getRange(rowIndex, 3).setValue(body.loja || "");
    shP.getRange(rowIndex, 4).setValue(body.solicitante || "");
    shP.getRange(rowIndex, 5).setValue("Aberto");
    shP.getRange(rowIndex, 7).setValue("");
    shP.getRange(rowIndex, 8).setValue(itensIn.length);
    shP.getRange(rowIndex, 9).setValue(0);

    // remove itens antigos deste pedido
    var shI = getSheet(SHEET_ITENS);
    var lastI = shI.getLastRow();
    if (lastI >= 2) {
      var idsI = shI.getRange(2, 1, lastI - 1, 2).getValues();
      for (var j = idsI.length - 1; j >= 0; j--) {
        if (String(idsI[j][1]) === String(pedidoId)) {
          shI.deleteRow(j + 2);
        }
      }
    }
